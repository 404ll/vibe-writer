...
def test_append_multiple_events_preserves_order():
    ...
    job_id = str(uuid.uuid4())
    store.create_job(job_id=job_id, topic="测试主题")
    store.append_event(job_id, SSEEvent(event="stage_update", data={"stage": "plan"}))
    store.append_event(job_id, SSEEvent(event="outline_ready", data={"outline": ["章节一"]}))
    store.append_event(job_id, SSEEvent(event="done", data={}))
    events = store.get_events(job_id)
