...
const databaseName = `vibe_writer_memory_api_${canaryId}`
const apiRoleName = `vibe_writer_api_${canaryId.slice(0, 16)}`
...
try {
  temporaryRoot = await mkdtemp(join(tmpdir(), 'vibe-writer-memory-api-'))
  ...
  await run('psql', [
    ...
    '-d', databaseName,
    '-v', 'ON_ERROR_STOP=1',
    '-c', `COMMENT ON DATABASE ${databaseName} IS 'vibe-writer-memory-api-canary:${canaryId}'`,
  ])
  ...
  const apiUrl = `postgres://${apiRoleName}@127.0.0.1:${port}/${databaseName}`
  await run('pnpm', ['--filter', '@vibe-writer/web', 'test:memory-api-canary'], {
    env: {
      TEST_DATABASE_URL: ownerUrl,
      TEST_DATABASE_API_URL: apiUrl,
      TEST_DATABASE_API_ROLE: apiRoleName,
      VIBE_WRITER_MEMORY_API_CANARY_ID: canaryId,
    },
  })
} finally {
  let stopError
  ...
  if (postgresStillRunning) {
    throw new Error(
      `Memory API canary PostgreSQL is still running; preserved ${temporaryRoot}`,
      { cause: stopError },
    )
  }
}
