import { ListMemoryCandidatesResponseSchema } from '@vibe-writer/contracts/memory/management/candidates'
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const page = await getWorkspaceDurableRepositories(authorization.scope)
      .memory.listCandidatesPage({
        limit: query.data.limit,
        ...(cursor ? { cursor } : {}),
      })
    return Response.json(
      ListMemoryCandidatesResponseSchema.parse({
        candidates: page.items.map(toMemoryCandidate),
