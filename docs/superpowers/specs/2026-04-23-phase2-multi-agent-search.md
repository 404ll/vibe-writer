...
完整架构（来自 `docs/superpowers/specs/2026-04-23-vibe-writer-design.md`）：

```
Orchestrator（协调者）
    ├── PlannerAgent     → 调用 LLM 生成大纲，解析章节列表
    ├── SearchAgent      → 搜索章节关键词 + LLM 提炼参考资料       ← Phase 2 新增
    ├── WriterAgent      → 根据章节标题 + 参考资料写正文
    └── ReviewAgent      → 轻审（每章）+ 重审（全文）               ← Phase 3
```

**Phase 2 范围：** 仅引入 PlannerAgent、SearchAgent、WriterAgent，重构 Orchestrator 为协调者。ReviewAgent 留到 Phase 3。
