...
Iteration 0058继续拆分write plane。outbox dispatcher使用独立`DATABASE_WRITE_DISPATCHER_URL`和`NOBYPASSRLS` role，只能`SELECT/UPDATE outbox_events`；consumer使用另一条`DATABASE_WRITE_CONSUMER_URL`和显式`BYPASSRLS` service role，只能访问Job lease/run/effect/trace/terminal/checkpoint attempt、outline command、Article初次写入以及`langgraph_checkpoint` fenced saver所需DML。Iteration 0062移除post-run Memory投递后，consumer也不再拥有`outbox_events` INSERT。`all`只表示进程拓扑合并，数据库身份仍是两个连接，配置会拒绝相同URL或role。
